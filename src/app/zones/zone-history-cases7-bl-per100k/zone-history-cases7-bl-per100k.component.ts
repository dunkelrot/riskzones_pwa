...
@Component({
  selector: 'app-zone-history-cases7-bl-per100k',
  templateUrl: './zone-history-cases7-bl-per100k.component.html',
  styleUrls: ['./zone-history-cases7-bl-per100k.component.css']
})
export class ZoneHistoryCases7BlPer100kComponent implements OnInit {
    ...
    for (let ii = 0; ii < numEntries; ii++) {
      ...
      zoneHistory.records.forEach((record) => {
        if (record.dateTime.toMillis() === date.toMillis()) {
          dataEntry.value = record.cases7BlPer100k;
        }
      });
